...
class Hadits extends CI_Controller {
	...
	function ResultSummarizing(){
		...
		// }
		if($forum == 'mashara'){
			$command = escapeshellcmd("sudo python3 file/py/result_mashara.py ".$url);
			$output = shell_exec($command);
		}else if($forum == 'other1'){
			$command = escapeshellcmd("sudo python3 file/py/result_other1.py ".$url);
			$output = shell_exec($command);
		}else{
			$command = escapeshellcmd("sudo python3 file/py/result_other2.py ".$url);
			$output = shell_exec($command);
		}
		var_dump($command);
		var_dump($output);
